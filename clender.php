...
function reservation($date,$reservation_array){
    require('dbconnect.php');
    $memos = $db->prepare('SELECT * FROM cl where day=?'); #ANDを入れてmember_idを入れる
    $memos->execute(array($date));
    $memo = $memos->fetch();
    
    if($memo){
        if(array_key_exists($date,$reservation_array)){
            $reservation_member = "<br/>"."<a href='edit.php?id=". $memo['id']."'>".$reservation_array[$date]."</a>";
            return $reservation_member;
        }
...
<header>
    <h1>
        <a href="clender.php"><img src="logo2.png" alt="log"></a> 
    </h1>
</header>
<nav>
<details>
　<summary>クリックして変更</summary>
　　<li><a href="">Myページ</a></li>
　　<li><a href="share.php">共有</a></li>
</details>
</nav>
